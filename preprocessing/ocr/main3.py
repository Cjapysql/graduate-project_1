...
IMAGE_ROOT_DIR = SRC_DIR / "im"

OCR_OUTPUT_DIR = SRC_DIR / "json_data_ocr"
OCR_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

if not IMAGE_ROOT_DIR.exists():
    raise RuntimeError(f"❌ 폴더가 존재하지 않습니다: {IMAGE_ROOT_DIR}")

# =========================
# 에피소드(화) 폴더 수집
# =========================
episode_dirs = sorted(
    [d for d in IMAGE_ROOT_DIR.iterdir() if d.is_dir()],
...
for episode_dir in episode_dirs:
    ...
    if not image_files:
        print(f"⚠ {episode_dir.name}화에 이미지 없음, 스킵")
        continue

    for img_path in tqdm(image_files, desc=f"🔍 {episode_dir.name}화 OCR"):
